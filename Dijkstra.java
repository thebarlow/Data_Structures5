...
public class Dijkstra {
  ...
  public void computeAllEuclideanDistances() {
	  Collection<Vertex> allCity = getVertices();
	  for(Vertex i : allCity){//run through all cities
		  List<Edge> adjCity=i.adjacentEdges;
		  for(Edge e :adjCity){//run through every city adjacent
			  System.out.print("Barlows stoopid");
  ...
  public void doDijkstra(String s) {
	  ...
	  while(unKnown>0){
		  Vertex temp=null;
		  for(Vertex v:allVertex){
			  if(!v.known && v.distance!=Double.POSITIVE_INFINITY){
				  if(temp==null){
					  temp=v;
				  }
				  if(v.distance<temp.distance){
					  temp=v;
				  }
			  }
		  }
		  ...
		  List<Edge> adj = temp.adjacentEdges;
		  for(Edge e: adj){
			  Vertex target=e.target;
			  if(!target.known){
				  double cost=e.distance;
				  if(temp.distance+cost<target.distance){
					  target.distance=temp.distance+cost;
					  target.prev=temp;
				  }
			  }
		  }
